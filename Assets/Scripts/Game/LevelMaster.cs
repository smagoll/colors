...
public class LevelMaster : MonoBehaviour
{
    ...
    {
        ParticleManager.instance.IsFirework = false;
        
        letterSpawner.Launch(level);
    ...
    public void Clue()
    {
        while (true)
        {
            ...
            foreach (var word in Level.Words)
            {
                ...
                {
                    if (!CheckDeckWord(deck, word)) continue;
    ...
    private bool CheckDeckWord(Deck deck, string word) // true, если слово можно использовать в колоде
    {
        if (deck.Cells.Count != word.Length) return false;
        var doneCell = deck.Cells.FirstOrDefault(x => x.IsDone);
        if (doneCell == null) return true;
        var index = word.IndexOf(doneCell.InstalledLetter.Symbol);
        if (index < 0)
            return false;
        if (deck.Cells[index].InstalledLetter.Symbol == word[index])
            return true;
        return false;
    }
    #endregion
